...
function p(){p=Object.assign||function(c){for(var f=1;f<arguments.length;f++){var g=arguments[f],m;for(m in g)Object.prototype.hasOwnProperty.call(g,m)&&(c[m]=g[m]);}return c};return p.apply(this,arguments)}var u,x=u||(u={});x.Pop="POP";x.Push="PUSH";x.Replace="REPLACE";var A=function(c){return Object.freeze(c)};function B$1(c,f){if(!c){"undefined"!==typeof console&&console.warn(f);try{throw Error(f);}catch(g){}}}
...
retry:function(){t(-1*k);}},t(k));}else B$1(!1,"You are trying to block a POP navigation to a location that was not created by the history library. The block will fail silently in production, but in general you should do all navigation with the history library (instead of using window.history.pushState directly) to avoid this situation.");else y(a);}});var z=u.Pop;c=f();var n=c[0],v=c[1],D=G(),l=G();null==n&&(n=0,r.replaceState(p({},r.state,{idx:n}),""));return {get action(){return z},
get location(){return v},createHref:g,push:E,replace:F,go:t,back:function(){t(-1);},forward:function(){t(1);},listen:function(a){return D.push(a)},block:function(a){var b=l.push(a);1===l.length&&q.addEventListener("beforeunload",C);return function(){b();l.length||q.removeEventListener("beforeunload",C);}}}}();const _history = H;
...
const store = new class Store {
    ...
    */
     ...
     */
    has(url, has) {
        return !has?.snapshot ? this.cache.has(url) : (this.cache.has(url) ||
...
var r,B=r||(r={});B.Pop="POP";B.Push="PUSH";B.Replace="REPLACE";function I(b){var h=b.pathname;h=void 0===h?"/":h;var e=b.search;e=void 0===e?"":e;b=b.hash;b=void 0===b?"":b;e&&"?"!==e&&(h+="?"===e.charAt(0)?e:"?"+e);b&&"#"!==b&&(h+="#"===b.charAt(0)?b:"#"+b);return h}
...
function parse$1(link) {
    const location = J(link instanceof Element ? link.getAttribute('href') : link);
    return {
        lastpath: I(_history.location),
        search: '',
        origin,
        hostname,
        ...location
...
function jsonattrs(accumulator, current, index, source) {
    ...
    return (index % 2 ? ({
        ...accumulator,
        [source[prop]]: isNumber.test(current)
            ? Number(current)
            : current
    }) : accumulator);
}
...
const loadjs = loadjs_umd.exports;
 ...
 */
async function capture({ url, snapshot }) {
    if (store.has(url, { snapshot: true })) {
 ...
 * Popstate Navigation
 */
async function popstate(url, state) {
    if (url !== inTransit)
        cancel(inTransit);
    if (store.has(url, { snapshot: true })) {
...
function listener({ action, location }) {
    if (action === 'POP') {
        return popstate(I(location), location.state);
    }
}
/**
...
function updateState() {
    _history.replace(_history.location, { ..._history.location.state, position });
 ...
 */
async function navigate(url, state = false) {
    if (state) {
...
const visit = (link, state = {}) => {
    ...
    return navigate(url, { ...state, url, location });
};
/**
 * Disconnect
 */
